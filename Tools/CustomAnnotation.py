...
CSV_HEADER = ['Chrom', 'Pos', 'Ref', 'Alt', 'AC', 'Gene', 'GeneName', 'GeneFunc', 'ExonicFunc', 'AAchange', 'ExAC', 'gnomAD_genome', 'VariantType', 'MetaSVM', 'CADD',
		'PP2', 'MCAP', '1KG', 'mis_z', 'lof_z', 'pLI', 'pRec', 'HeartRank', 'LungRank', 'BrainRank', 'Filter', 'QUAL', 'ProbandGT', 'FatherGT', 'MotherGT']
...
class Record():
	...
	def read_PsapReport(self, l, header, Ped):
		...
		self.CADDphred = llist[header.index('CADD_Phred')]
		self.DzModel = llist[header.index('Dz.Model.' + Ped.Proband.Sample)]
		self.Pval = llist[header.index('popScore.' + Ped.Proband.Sample)]
		self.Flag = llist[header.index('Flag')]
	...
	def FormRecord(self, Ped, genescore):
		...
		GeneFunc = self.Info['Func.refGene'][idx]
		ExonicFunc = self.Info['ExonicFunc.refGene'][idx]
		AAchange = self.Info['AAchange'][idx]
		ExAC_ALL = AF(self.Info['ExAC_ALL'][idx])
		gnomAD_genome_ALL = AF(self.Info['gnomAD_genome_ALL'][idx])
		MCAP = self.Info['MCAP'][idx]
		MetaSVM = self.Info['MetaSVM_pred'][idx]
		CADD = self.Info['CADD_phred'][idx]
		PP2 = self.Info['Polyphen2_HDIV _pred'][idx]
		VarType = GetVarType(MetaSVM, CADD, PP2)
		_1KG = AF(self.Info['1000g2015aug_all'][idx])
		...
		self.VarType = self.VCF.GetVarType()
