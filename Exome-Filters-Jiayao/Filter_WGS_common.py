...
def GetOptions():
	...
	parser.add_argument('-f', '--freq', type=float, default=5e-3,
			help="WGS AF greater than freq (default = 5e-4)")
...
class WGS_Common:
	def __init__(self, args):
		...
		self.freq = args.freq
		if self.out == None:
			self.out = self.vcf.split(
					"/")[-1].rstrip(".gz").rstrip(".vcf") + ".wgscommon.vcf"
	...
	def run(self):
		...
		for l in hand:
			if l.startswith('##'):
				...
				self.NumOfIndv = len(l.strip().split()) - 8
				fout.write(l)
				continue
			llist = l.strip().split('\t')
			Count_All += 1
		...
		for kv in infolist:
			...
			if len(kv) == 2:
				...
				if k in infodict:
					...
					infodict[k] = v
		CallRate = float(infodict['AN']) / self.NumOfIndv
		if CallRate < 0.95:
			return False
		AF = infodict['gnomAD_genome_ALL'].split(',')[0]
		if AF == '.':
			return False
		if (float(AF) < self.freq) :#or (float(infodict['FS'] > 15)):# or (float(infodict['QD'] < 2)):
			return False
		else:
			#print float(AF),float(infodict['FS']), float(infodict['QD'])
			return True
