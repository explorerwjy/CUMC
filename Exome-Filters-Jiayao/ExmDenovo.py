...
def GetOptions():
	...
	parser.add_argument("-p", "--ped", type=str, required=True, help="<Required> PED file")
	parser.add_argument("-f", "--filter", type=str, help="ymal file contains filter criteria")
	args = parser.parse_args()
	if args.filter == None:
		args.filter = DENOVO_FILTER
	return args.vcf, args.ped, args.filter
...
def Call_DeNovo(vcf, ped, yaml_fname):
...
def main():
	vcf, ped, yaml_fname = GetOptions()
	Call_DeNovo(vcf,ped, yaml_fname)
