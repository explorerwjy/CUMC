...
class Sample():
	...
	def __init__(self,name,Format,data):
		...
		self.format = Format.split(':') # Format ["GT", "AD", "DP", "GQ", "PL"]
		self.data = data.split(":") # Data ["0/0", "7,0", "7", "18", "0,18,270"]
		self.dict = {}
		for i,item in enumerate(self.format):
			self.dict[item] = self.data[i]
			if self.dict['GT'] == './.': # If sample don't have genotpye, no need to parse further
				break 
		self.GT = re.findall('[\d.]',self.dict['GT']) #GT '0/0' --> ['0', '0']
	def get_format(self):
...
class INFO():
	def __init__(self,alts,raw_string):
		self.get_unique(raw_string) #This INFO group is unique for each site. Come from variants caller or other annotations.
		self.get_annovar(alts,raw_string) #This INFO group is unique for each alt allele, Come from ANNOVAR annotation that give each allele one annotation.

	# A set of commen annotation is before the first "ANNOVAR_DATE" and after last "ALLELE_END"
...
class Variant():
	...
	def GetSample(self,record):
		...
		for i,sample in enumerate(self.headers[9:]):
			#print headers
			#print record
			self.Sample[sample] = Sample(sample,self.Format,self.record[i+9])
...
class Pedigree():
	def __init__(self,line):
		self.fam,self.proband,self.father,self.mother = line.strip().split('\t')[0:4]
		self.out_name = '_'.join([self.proband,self.father,self.mother]) + '.vcf'
