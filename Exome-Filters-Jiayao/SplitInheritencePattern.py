...
def GetOptions():
    ...
    parser.add_option('-d', '--dir', dest='dir', metavar='dir',
                      help='InputDir contains VCF file and pedigree files')
    parser.add_option('-v', '--vcf', dest='vcf',
                      help='Inhretence annotated VCF file')
...
def LookCompoundHeter(CH_buffer, fout_CH, RecordLen, Format_idx, headerList, Ped):
    ...
    if RecordLen - Format_idx - 1 == 3:  # Trios
        Proband_idx, Father_idx, Mother_idx = [
            Format_idx + i + 1 for i in range(3)]
...
def selectAD_DN(llist, headerList, Ped, Model, Format_idx):
    ...
    if 'dn' in Model:
        try:
            ...
            # print Proband.fmt['AD'], int(Proband.fmt['GT'][1]),
            # float(Proband.fmt['AD'])
            if Proband.GT[1] == '.':
                return False
            ADDP = float(Proband.fmt['AD'][int(
                Proband.GT[1])]) / float(Proband.fmt['DP'])
...
def SplitInheritencePattern(InpDir, VCF, PED):
    ...
    for vcf, ped in vcf_peds:
        ...
        for l in fin:
            ...
            if Gene != LastGene:
                if LastGene != None:
                    LookCompoundHeter(CH_buffer, fout_CH,
                                      RecordLen, Format_idx, headerList, Ped)
                CH_buffer = [l]
                LastGene = Gene
            else:
                CH_buffer.append(l)
