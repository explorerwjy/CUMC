...
def LGD_Dmis(info):
	for annotation in info.annovar:
		...
		if flag == True:
			return True
	return False
def InGeneList(info,genelist):
	for annotation in info.annovar:
		flag = False
		try:
			if annotation['Gene.refGene'] in genelist:
				return True
		except KeyError:
			continue
	return False
def GetOptions():
	parser = OptionParser()
	parser.add_option('-v','--vcf',dest = 'VCF', metavar = 'VCF', help = 'Input VCF')
	parser.add_option('-g','--gene',dest = 'GENE', metavar = 'GENE', help = 'Input gene list')
	parser.add_option('-o','--out',dest = 'OUT', metavar = 'OUT', help = 'Output VCF')
	(options,args) = parser.parse_args()
	
	return options.VCF,options.GENE,options.OUT


def Filter_VCF(In,gene,Out):
	genes = [ item.strip() for item in open(gene,'rb').readlines()]
	print "Gene List is:"
	print genes
	if In.endswith('.gz'):
		fin = gzip.open(In,'rb')
	...
	fout.close()
def main():
	In,gene,Out = GetOptions()
	Filter_VCF(In,gene,Out)
