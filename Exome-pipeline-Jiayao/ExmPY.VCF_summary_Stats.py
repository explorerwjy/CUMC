...
parser.add_option("-F", "--filter", action='store_true', dest="FilterVars", help="Only count good quality variants")
...
BadFilters=['QD_Bad_SNP','FS_Bad_SNP','FS_Mid_SNP;QD_Mid_SNP','LowQD_Indel']
...
for WhichCode in CodingTypes:
    ...
    for line in VCF:
        ...
        if '#' not in line:
            ...
            #check whether it is a coding variant and then run only if appropriate
            if \
            ...
            WhichCode=="All variants":
                # first run a dummy sample to count the "total" number of variants, this is actually just the number of loci and ignores mulit-allelic factors, then loop through each sample column
                for SampleNumber in range(0, SamLength):
                    
                    #first get the genotype for the sample
                    SampleColumn=SampleNumber+8 
                    if SampleNumber==0:
                        #this is for the total so make the genotype the first alternate allele
                        GT="0/1"
                    else:
                    ...
                    #continue only if this is a variant (not 0/0 or uncalled) otherwise move on to the next sample
                    ...
                    if all( str(i) not in VariantFilter for i in BadFilters) or not FilterVariants:
                        if SNV: SNVcount[SampleNumber]=SNVcount[SampleNumber]+1
                        if Indel: InDelCount[SampleNumber]=InDelCount[SampleNumber]+1
                        if Known: KnownCount[SampleNumber]=KnownCount[SampleNumber]+1
                        if Novel: NovelCount[SampleNumber]=NovelCount[SampleNumber]+1
                        if Known and Transition: KnownTiCount[SampleNumber]=KnownTiCount[SampleNumber]+1
                        if Known and Transversion: KnownTvCount[SampleNumber]=KnownTvCount[SampleNumber]+1
                        if Novel and Transition: NovelTiCount[SampleNumber]=NovelTiCount[SampleNumber]+1
                        if Novel and Transversion: NovelTvCount[SampleNumber]=NovelTvCount[SampleNumber]+1
                        if Silent: SilentCount[SampleNumber]=SilentCount[SampleNumber]+1
                        if Missense: MissenseCount[SampleNumber]=MissenseCount[SampleNumber]+1
                        if Nonsense: NonsenseCount[SampleNumber]=NonsenseCount[SampleNumber]+1
                        if Unknown: UnknownCount[SampleNumber]=UnknownCount[SampleNumber]+1
                        if FrameShift: FrameShiftCount[SampleNumber]=FrameShiftCount[SampleNumber]+1
                        if Homozygous: HomozygousCount[SampleNumber]=HomozygousCount[SampleNumber]+1
                        if Heterozygous: HeterozygousCount[SampleNumber]=HeterozygousCount[SampleNumber]+1
                        if WildType: WildTypeCount[SampleNumber]=WildTypeCount[SampleNumber]+1
                        if UnCalled: UnCalledCount[SampleNumber]=UnCalledCount[SampleNumber]+1
                        if Rare: RareCount[SampleNumber]=RareCount[SampleNumber]+1
                        if Silent and Rare: RareSilentCount[SampleNumber]=RareSilentCount[SampleNumber]+1
                        SNV=False
                        Indel=False
                        Transversion=False
                        Transition=False
    ...
    Output.write("\t".join(['Overall Ti/TV ratio']+[ str(i) for i in TotalTiTvRat ])+"\n")
    ...
    Output.write("\t".join(['Reference']+[ str(i) for i in WildTypeCount ])+"\n")
    Output.write("\t".join(['Not Called']+[ str(i) for i in UnCalledCount ])+"\n")
    Output.write("\t".join(['Rare (AAF<0.01 or missing in ExAC)']+[ str(i) for i in RareCount ])+"\n")
    if str(CodingTypes)!='Non-coding Variants':
        Output.write("\t".join(['Rare Silent']+[ str(i) for i in RareSilentCount ])+"\n")
    Output.write("\t\n")
