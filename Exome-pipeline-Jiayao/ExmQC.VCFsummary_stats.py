...
class stats():
	def __init__(self,line,WhichCode):
		...
		self.ESPRareCount=[0]*self.SamLength
...
def GetOptions():
	...
	parser.add_option("-q","--qc_folder",dest="QC_folder",help="User specified name of folder for output to be written", metavar="QC_folder", default="QC_summary")
...
def GetFrequency(INFOdict):
	...
	return KGscore,ESPscore,ExACscore
	...
	for tmp in STATS:	
		for i in range(0, tmp.SamLength):
			if int(tmp.knownTvCount[i])>0:
				tmp.KnownTiTvRat[i]=float(tmp.knownTiCount[i])/float(tmp.knownTvCount[i])
			if int(tmp.novelTvCount[i])>0:
				tmp.NovelTiTvRat[i]=float(tmp.novelTiCount[i])/float(tmp.novelTvCount[i])
			tmp.TotalTvCount=float(tmp.novelTvCount[i])+float(tmp.knownTvCount[i])
			tmp.TotalTiCount=float(tmp.novelTiCount[i])+float(tmp.knownTiCount[i]) 
			if tmp.TotalTvCount>0:
				tmp.TotalTiTvRat[i]=float(tmp.TotalTiCount)/float(tmp.TotalTvCount)
		#write output
		Output.write("\t".join([str(tmp.WhichCode)])+"\n")
		Output.write("\t".join(['Samples:']+[ str(i) for i in tmp.Samples ]))
		Output.write("\t".join(['SNVs']+[ str(i) for i in tmp.SNVcount ])+"\n")
		Output.write("\t".join(['InDels']+[ str(i) for i in tmp.InDelCount ])+"\n")
		Output.write("\t".join(['Known']+[ str(i) for i in tmp.KnownCount ])+"\n")
		Output.write("\t".join(['Novel']+[ str(i) for i in tmp.NovelCount ])+"\n")
		Output.write("\t".join(['known Ti']+[ str(i) for i in tmp.knownTiCount ])+"\n")
		Output.write("\t".join(['known TV']+[ str(i) for i in tmp.knownTvCount ])+"\n")
		Output.write("\t".join(['known Ti/TV ratio']+[ str(i) for i in tmp.KnownTiTvRat ])+"\n")
		Output.write("\t".join(['novel Ti']+[ str(i) for i in tmp.novelTiCount ])+"\n") 
		Output.write("\t".join(['novel TV']+[ str(i) for i in tmp.novelTvCount ])+"\n")
		Output.write("\t".join(['novel Ti/TV ratio']+[ str(i) for i in tmp.NovelTiTvRat ])+"\n")
		Output.write("\t".join(['total Ti/TV ratio']+[ str(i) for i in tmp.TotalTiTvRat ])+"\n")
		if str(tmp.WhichCode)!='Non-coding Variants':
			Output.write("\t".join(['Silent']+[ str(i) for i in tmp.silentCount ])+"\n")
			Output.write("\t".join(['Missense']+[ str(i) for i in tmp.missenseCount ])+"\n")
			Output.write("\t".join(['Nonsense']+[ str(i) for i in tmp.nonsenseCount ])+"\n")
			Output.write("\t".join(['Unknown']+[ str(i) for i in tmp.unknownCount ])+"\n") 
			Output.write("\t".join(['Frameshift']+[ str(i) for i in tmp.frameShiftCount ])+"\n")
			Output.write("\t".join(['Homozygous']+[ str(i) for i in tmp.homozygousCount ])+"\n")
			Output.write("\t".join(['Heterozygous']+[ str(i) for i in tmp.heterozygousCount ])+"\n")
		Output.write("\t".join(['Rare (AAF<0.01) - eKG']+[ str(i) for i in tmp.KGRareCount ])+"\n")
		Output.write("\t".join(['Rare (AAF<0.01) - ExAC']+[ str(i) for i in tmp.KGRareCount ])+"\n")
		Output.write("\t".join(['Rare (AAF<0.01) - ESP']+[ str(i) for i in tmp.ESPRareCount ])+"\n")
		Output.write("\t".join(['Rare (AAF<0.01) - 1KG or ESP']+[ str(i) for i in tmp.RareCount ])+"\n")
		Output.write("\t".join(['No AAF']+[ str(i) for i in tmp.NoAAF ])+"\n")
		Output.write("\t".join(['not called']+[ str(i) for i in tmp.UnCalled ])+"\n")
		Output.write("\t\n")
	Output.close()
...
def Summary(VCF,QCdir,Nprocs):
	...
	for line in fin:
		if line.startswith('#CHROM'):
			...
			Indel = not (all(i in Nucleotides for i in REF) and all(i in Nucleotides for i in ALT))
			Transition = (REF[0]=='A' and ALT[0]=='G') or (REF[0]=='G' and ALT[0]=='A') or (REF[0]=='C' and ALT[0]=='T') or (REF[0]=='T' and ALT[0]=='C')
	...
	for i in xrange(2,len(All.Samples)):
		for group in [coding,noncoding,All]:
			group.SNVcount[1] = sum(group.SNVcount[2:])
			group.InDelCount[1] = sum(group.InDelCount[2:])
			group.KnownCount[1] = sum(group.KnownCount[2:])
			group.NovelCount[1] = sum(group.NovelCount[2:])
			group.knownTiCount[1] = sum(group.knownTiCount[2:])
			group.knownTvCount[1] = sum(group.knownTvCount[2:])
			group.novelTiCount[1] = sum(group.novelTiCount[2:])
			group.novelTvCount[1] = sum(group.novelTvCount[2:])
			group.silentCount[1] = sum(group.silentCount[2:])
			group.missenseCount[1] = sum(group.missenseCount[2:])
			group.nonsenseCount[1] = sum(group.nonsenseCount[2:])
			group.unknownCount[1] = sum(group.unknownCount[2:])
			group.frameShiftCount[1] = sum(group.frameShiftCount[2:])
			group.homozygousCount[1] = sum(group.homozygousCount[2:])
			group.heterozygousCount[1] = sum(group.heterozygousCount[2:])
			group.referenceCount[1] = sum(group.referenceCount[2:])
			group.KGRareCount[1] = sum(group.KGRareCount[2:])
			group.ESPRareCount[1] = sum(group.ESPRareCount[2:])
			group.RareCount[1] = sum(group.RareCount[2:])
			group.ExACRareCount[1] = sum(group.ExACRareCount[2:])
			group.NoAAF[1] = sum(group.NoAAF[2:])
			group.UnCalled[1] = sum(group.UnCalled[2:])
