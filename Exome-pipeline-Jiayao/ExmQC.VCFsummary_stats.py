...
def Summary(VCF, QCdir, Nprocs):
    ...
    for i in xrange(2, len(All.Samples)):
        for group in [coding, noncoding, All]:
            group.SNVcount[1] = sum(group.SNVcount[2:])/len(All.Samples)
            group.InDelCount[1] = sum(group.InDelCount[2:])/len(All.Samples)
            group.KnownCount[1] = sum(group.KnownCount[2:])/len(All.Samples)
            group.NovelCount[1] = sum(group.NovelCount[2:])/len(All.Samples)
            group.knownTiCount[1] = sum(group.knownTiCount[2:])/len(All.Samples)
            group.knownTvCount[1] = sum(group.knownTvCount[2:])/len(All.Samples)
            group.novelTiCount[1] = sum(group.novelTiCount[2:])/len(All.Samples)
            group.novelTvCount[1] = sum(group.novelTvCount[2:])/len(All.Samples)
            group.silentCount[1] = sum(group.silentCount[2:])/len(All.Samples)
            group.missenseCount[1] = sum(group.missenseCount[2:])/len(All.Samples)
            group.nonsenseCount[1] = sum(group.nonsenseCount[2:])/len(All.Samples)
            group.unknownCount[1] = sum(group.unknownCount[2:])/len(All.Samples)
            group.frameShiftCount[1] = sum(group.frameShiftCount[2:])/len(All.Samples)
            group.homozygousCount[1] = sum(group.homozygousCount[2:])/len(All.Samples)
            group.heterozygousCount[1] = sum(group.heterozygousCount[2:])/len(All.Samples)
            group.referenceCount[1] = sum(group.referenceCount[2:])/len(All.Samples)
            group.KGRareCount[1] = sum(group.KGRareCount[2:])/len(All.Samples)
            group.gnomADRareCount[1] = sum(group.gnomADRareCount[2:])/len(All.Samples)
            group.RareCount[1] = sum(group.RareCount[2:])/len(All.Samples)
            group.ExACRareCount[1] = sum(group.ExACRareCount[2:])/len(All.Samples)
            group.NoAAF[1] = sum(group.NoAAF[2:])/len(All.Samples)
            group.UnCalled[1] = sum(group.UnCalled[2:])/len(All.Samples)
