...
PSAP_PATH=/home/local/users/jw/CUMC/psap
...
# Check that all required ANNOVAR annotation files are in the humandb directory
        MISSING=0
        for FILE in "hg19_ALL.sites.2015_08.txt" "hg19_cadd13.txt" "hg19_esp6500siv2_all.txt" "hg19_avsnp147.txt" "hg19_wgEncodeGencodeBasicV19Mrna.fa" "hg19_wgEncodeGencodeBasicV19.txt" "hg19_mac63kFreq_ALL.txt"
        do
...
# Write column names from VCF file to header file (will be used later)
        less $VCF|head -n 1000| grep '#' | tail -n 1 > ${OUTFILE}.avinput.header # Extract all lines of the VCF header.  The last line of the VCF header contains coumn names - write columna names to .avinput.header file
...
# Annotate with ANNOVAR
	...
	if [ ! -f annotated/${OUTFILE}.avinput.hg19_multianno.txt ]; then 
	perl ${ANNOVAR_PATH}table_annovar.pl ${OUTFILE}.avinput -remove -outfile annotated/${OUTFILE}.avinput ${ANNOVAR_PATH}humandb/ -buildver hg19 -protocol wgEncodeGencodeBasicV19,mac63kFreq_ALL,esp6500siv2_all,ALL.sites.2015_08,avsnp147,cadd13 -operation g,f,f,f,f,f -nastring NA -otherinfo -argument -separate,,,,,-otherinfo
   	fi
...
# RUN apply_popStat_individual.R for each individual
	...
	for i in $IDS
	do	
		Rscript ${PSAP_PATH}/RScripts/apply_popStat_individual.R ${OUTFILE}.avinput $i $PSAP_PATH $PED_FILE &
		if [ `expr $IDX % 10` -eq 0 ]
		then
